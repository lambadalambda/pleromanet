...
- Final verification passed all 288 route Playwright tests with one worker, `pnpm check`, and the production build. Repository-wide Playwright discovery remains blocked by the pre-existing direct-runner inability to resolve `$app/paths` from library tests; the complete route suite is unaffected.
